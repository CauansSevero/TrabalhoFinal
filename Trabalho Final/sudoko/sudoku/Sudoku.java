...
class Sudoku {
    ...
    public void jogar(char coluna, char linha, char valor) {
        ...
        if (tabuleiro[i - 1][j - 1] == 0) {
            if (verificarLinha(i - 1, v)) {
                if (verificarColuna(j - 1, v)) {
                    if(verificarQuadrante(i - 1, j - 1, v)){
                        tabuleiro[i - 1][j - 1] = v;
                    }else{
                        System.out.println("já tem no quadrante");
                    }
                } else {
                    System.out.println("já tem na coluna");
                }
            } else {
                System.out.println("já tem na linha");
            }
        } else {
    ...
    public boolean verificarLinha(int linha, int valor) {
        for (int i = 0; i < tabuleiro.length; i++) {
            //System.out.println(valor + " == " + tabuleiro[linha][i] + " ? " + (valor == tabuleiro[linha][i]));
    ...
    public boolean verificarColuna(int coluna, int valor) {
        for (int i = 0; i < tabuleiro.length; i++) {
            //System.out.println(valor + " == " + tabuleiro[i][coluna] + " ? " + (valor == tabuleiro[i][coluna]));
    ...
    public boolean verificarQuadrante(int i, int j, int v) {
        boolean valido = true;
        int li = i - (i % 3);
        int lj = j - (j % 3);
        for (int k = li; k < li + 3; k++) {
            for (int l = lj; l < lj + 3; l++) {
                if (tabuleiro[k][l] == v) {
                    valido = false;
                }
            }
